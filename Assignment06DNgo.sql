...
-- 2017-05-27,DNgo,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment06DB_DNgo')
	 Begin 
	  Alter Database [Assignment06DB_DNgo] set Single_user With Rollback Immediate;
	  Drop Database Assignment06DB_DNgo;
	 End
	Create Database Assignment06DB_DNgo;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment06DB_DNgo;
...
CREATE or ALTER VIEW vProducts
 WITH SCHEMABINDING
  AS
   SELECT
    Product ID, ProductName, CategoryID, UnitPrice
	 FROM dbo.Products;
GO

CREATE or FILTER VIEW vEmployees
 WITH SCHEMABINDING
  AS
   SELECT 
...
CREATE or FILTER VIEW vInventories
 WITH SCHEMABINDING
  AS
  SELECT 
...
CREATE or ALTER VIEW vInventoriesByEmployeesByDates
 AS
  SELECT DISTINCT 50000
   I.InventoryDate,
   E.EmployeeFirstName + ' ' + E.EmployeeLastName as EmployeeLastName
...
CREATE or ALTER VIEW vInventoriesByProductsByCategories
 AS
  SELECT TOP 50000
   ...
   I.COUNT
 FROM
  INNER JOIN v.Employees as F
   ON T.EmployeeID = EmployeeID
  INNER JOIN vProducts as P
